/** ======================================================================================
  ...
  @brief steng1ax data acquisition HAL
...
#define STM_STENG1AX_REG_OUT_X_L_G         (0x22)
...
*****************************************************************************************
*/
static uint8_t steng1ax_wake_src = 0;

/* ------------------------------------------------------------------------------------ */
static sns_com_port_status_e
steng1ax_get_data( sns_dd_handle_s*    dd_handle,
  ...
  if( status == SNS_COM_PORT_STATUS_SUCCESS && wake_src != 0xff )
  {
    sns_com_port_data_vector_s data_vectors[] = 
    {
      { .reg_addr = STM_STENG1AX_REG_CTRL5,
        .buf_sz   = 2 },
      { .reg_addr = STM_STENG1AX_REG_OUT_X_L_G,
        .buf_sz   = 12 },
    };
    *num_samples = 1;
...
/* ------------------------------------------------------------------------------------ */
static sns_com_port_status_e
steng1ax_parse_eng_data( sns_dd_handle_s*        dd_handle,
                          ...
                          notify_accel_sample_cnt notify_cnt_fptr )
{
  ...
  {
    int i = 8; /* first 2 bytes are REG_CTRL1_XL and REG_CTRL2_G, next 6 are ... data */
...
sns_dd_if_s steng1ax_hal_table =
  { .get_data = steng1ax_get_data,
    .parse_accel_data = steng1ax_parse_eng_data };
