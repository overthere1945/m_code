...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    boolean mCameraKeyDown = false;
    boolean mFunctionKeyDown = false;
...
// QTI_END: 2019-06-24: Display: frameworks/base: Add HDMI hotplug handling
    ...
    private Vibrator mCameraKeyVibrator;

    private static final AudioAttributes CAMERA_KEY_HAPTIC_ATTRIBUTES =
...
/**
 * KEY_CAMERA 입력 시 햅틱 피드백을 발생시키는 헬퍼 메서드.
 *
 * 주의:
 * - key event dispatch 흐름에는 관여하지 않음
 * - ACTION_DOWN 최초 1회에서만 호출하는 것을 전제로 함
 */
private void performCameraKeyHaptic() {
    if (mCameraKeyVibrator == null) {
        mCameraKeyVibrator =
                (Vibrator) mContext.getSystemService(Context.VIBRATOR_SERVICE);
    }

    if (mCameraKeyVibrator == null || !mCameraKeyVibrator.hasVibrator()) {
        return;
    }
    ...
    try {
        ...
        mCameraKeyVibrator.vibrate(effect, CAMERA_KEY_HAPTIC_ATTRIBUTES);
    } catch (Exception e) {
        Slog.w(TAG, "Failed to perform KEY_CAMERA haptic", e);
    }
}
    ...
    void init(Injector injector) {
        ...
        mCameraKeyVibrator = (Vibrator) mContext.getSystemService(Context.VIBRATOR_SERVICE);
    ...
    private void openCameraFromPuiCapture() {
    try {
        // 1. Intent 생성 및 명시적 컴포넌트(패키지명, 클래스명) 설정
        Intent intent = new Intent("com.lge.wearcamera.OPEN_CAMERA");
        intent.setClassName("com.lge.wearcamera", "com.lge.wearcamera.MainActivity");
        
        // 2. Extra 데이터 전달
        intent.putExtra("com.lge.wearcamera.ENTRY_SOURCE", "pui_capture");
        
        // 3. 시스템 레이어에서 Activity를 띄우기 위한 필수 플래그 추가
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK 
                | Intent.FLAG_ACTIVITY_SINGLE_TOP 
                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        
        Log.d(TAG, "HYL - com.lge.wearcamera.OPEN_CAMERA");
        // 4. PhoneWindowManager의 멤버 변수인 mContext와 mHandler를 활용하여
        // 현재 유저(UserHandle.CURRENT) 세션에서 비동기로 Activity 실행
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(TAG, "Failed to startActivityAsUser for wear camera", e);
                }
            }
        });

    } catch (Exception e) {
        Log.e(TAG, "Exception in openCameraFromPuiCapture", e);
    }
}

// PhoneWindowManager.java 내부 적절한 위치에 메서드 추가

private void openRecorderFromPuiCapture() {
    try {
        // 1. Intent 생성 및 명시적 컴포넌트(패키지명, 클래스명) 설정
        Intent intent = new Intent("com.lge.wearaudiorecorder.ACTION_START_RECORDER");
        intent.setClassName("com.lge.wearaudiorecorder", "com.lge.wearaudiorecorder.MainActivity");
        
        // 2. Extra 데이터 전달
        intent.putExtra("com.lge.wearaudiorecorder.ENTRY_SOURCE", "pui_capture");
        
        // 3. 시스템 레이어에서 Activity를 안정적으로 띄우기 위한 필수 플래그 추가
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK 
                | Intent.FLAG_ACTIVITY_SINGLE_TOP 
                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        
        Log.d(TAG, "HYL - com.lge.wearaudiorecorder.ACTION_START_RECORDER");
        // 4. 입력/윈도우 핵심 스레드 병목을 방지하기 위해 mHandler를 통해 비동기로 실행
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
                    // 현재 활성화된 유저(UserHandle.CURRENT) 세션에서 Activity 실행
                    mContext.startActivityAsUser(intent, UserHandle.CURRENT);
                } catch (Exception e) {
                    Log.e(TAG, "Failed to startActivityAsUser for wear audio recorder", e);
                }
            }
        });

    } catch (Exception e) {
        Log.e(TAG, "Exception in openRecorderFromPuiCapture", e);
    }
}

// PhoneWindowManager.java 내부 적절한 위치에 메서드 추가

private void openListeningFromPuiCapture() {
    try {
        // 1. Intent 생성 및 명시적 컴포넌트(패키지명, 클래스명) 설정
        Intent intent = new Intent("com.lge.wearhome.OPEN_LISTENING");
        intent.setClassName("com.lge.wearhome", "com.lge.wearhome.ui.MainActivity");
        
        // 2. Extra 데이터 전달 (전달받은 pui_listening 소스 명시)
        intent.putExtra("com.lge.wearhome.ENTRY_SOURCE", "pui_listening");
        
        // 3. 백그라운드 시스템 레이어에서 Activity를 즉각적이고 안정적으로 띄우기 위한 플래그 추가
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK 
                | Intent.FLAG_ACTIVITY_SINGLE_TOP 
                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
       
         Log.d(TAG, "HYL - com.lge.wearhome.OPEN_LISTENING"); 
        // 4. 입력 스레드의 병목 및 UI 얼어붙음(Freeze) 현상을 방지하기 위해 비동기 실행
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
                    // 현재 활성화된 유저(UserHandle.CURRENT) 세션 윈도우에 Activity 실행
                    mContext.startActivityAsUser(intent, UserHandle.CURRENT);
                } catch (Exception e) {
                    Log.e(TAG, "Failed to startActivityAsUser for wear home listening", e);
                }
            }
        });
...
private void openVisionSensingFromPuiCapture() {

         // --- CAMERA 키 처리 로직 시작 ---
             String targetPackage = "com.lge.weardatatransfer";
             String targetActivity = "com.lge.weardatatransfer.MainActivity";

             // 1. 화면이 꺼져 있다면 즉시 깨움
             if (!mPowerManager.isInteractive()) {
                 mPowerManager.wakeUp(SystemClock.uptimeMillis(),
                 PowerManager.WAKE_REASON_APPLICATION, "CameraKey:WakeUp");
             }
             ...
             try
             {
                 ...
                 // 중요도(Foreground) 상관없이 현재 실행 중인 모든 프로세스 확인
                 List<ActivityManager.RunningAppProcessInfo> appProcesses = am.getRunningAppProcesses();

                 if (appProcesses != null) {
                     for (ActivityManager.RunningAppProcessInfo appProcess : appProcesses) {
                         // 패키지 리스트 중에 해당 앱이 포함되어 있는지 확인
                 ...
                 if (isRunning) {
                      // 실행 중이면 강제 종료
                      //am.forceStopPackage(targetPackage);

                     Intent intent = new Intent("com.lge.weardatatransfer.action.GRACEFUL_SHUTDOWN");
                     intent.setComponent(new ComponentName(targetPackage, targetActivity));
                     intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

                     Log.d(TAG, "HYL - com.lge.weardatatransfer START"); 
                     // 시스템 사용자 권한으로 실행
                     mContext.startActivityAsUser(intent, UserHandle.CURRENT);

                      Log.d(TAG, "am.forceStopPackage com.lge.weardatatransfer");
                      Log.d(TAG, "HYL - com.lge.weardatatransfer STOP"); 
                 } else {
                     // 실행 중이 아니면 앱 시작
                     ...
                     // 시스템 사용자 권한으로 실행
                     mContext.startActivityAsUser(intent, UserHandle.CURRENT);
                     Log.d(TAG, "startActivityAsUser com.lge.weardatatransfer");
                }

final Context appContext = mContext.getApplicationContext(); // 안전한 Context 참조 보장
    new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(new Runnable() {
        @Override
        public void run() {
            // 핸들러 내부 런타임 에러 방지를 위한 자체 try-catch
            try {
                if (appContext != null) {
                    Intent intent2 = new Intent("com.lge.wearhome.TOGGLE_PRIVACY");
                    intent2.setClassName("com.lge.wearhome", "com.lge.wearhome.ui.MainActivity");
                    intent2.putExtra("com.lge.wearhome.ENTRY_SOURCE", "pui_privacy");
                    intent2.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP);

                    Log.d(TAG, "HYL - com.lge.wearhome START");
                    appContext.startActivityAsUser(intent2, UserHandle.CURRENT);
                }
            } catch (Exception e) {
                Log.e(TAG, "Error inside Handler for TOGGLE_PRIVACY", e);
            }
        }
    }, 500); // 0.5초 딜레이
                 ...
                 Log.e(TAG, "Error handling KEYCODE_CAMERA", e);
             }
    ...
    final int action = event.getAction();
         final boolean isLongPress = (event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0;
...
if (keyCode == KeyEvent.KEYCODE_CAMERA) {
        // 1. 카메라 버튼이 눌렸을 때 (ACTION_DOWN)
        if (action == KeyEvent.ACTION_DOWN) {
            if (repeatCount == 0) {
                // 첫 번째 다운 이벤트 시 이벤트를 소비하지 않고 프레임워크가 롱클릭을 계산할 수 있도록 통과시킵니다.
                mCameraKeyDown = true;
                return 0; 
            } else if (repeatCount == 2) { // 500ms * 2
                // 시스템이 롱클릭으로 판단한 순간 즉시 B Activity 실행
                openListeningFromPuiCapture();
                performCameraKeyHaptic();
                mCameraKeyDown = false;
                return keyConsumed; // 이벤트 소비 (더 이상 전파되지 않음)
            }
        } 
        // 2. 카메라 버튼에서 손을 뗐을 때 (ACTION_UP)
        else if (action == KeyEvent.ACTION_UP) {
            // 롱클릭 플래그가 없고, 취소되지 않은 정상적인 숏클릭인 경우
            if (!isLongPress && mCameraKeyDown && (event.getFlags() & KeyEvent.FLAG_CANCELED) == 0) {
             openVisionSensingFromPuiCapture();
                performCameraKeyHaptic();
             return keyConsumed; // 이벤트를 여기서 소비함
            }
            mCameraKeyDown = false;
        }
        return -1; // 기본 카메라 동작(순정 카메라 앱 실행 등)을 방지하기 위해 이벤트를 차단
    }
if (keyCode == KeyEvent.KEYCODE_FUNCTION) {
        ...
        if (action == KeyEvent.ACTION_DOWN) {
            if (repeatCount == 0) {
                // 첫 번째 다운 이벤트 시 이벤트를 소비하지 않고 프레임워크가 롱클릭을 계산할 수 있도록 통과시킵니다.
                mFunctionKeyDown = true;
                return 0; 
            } else if (repeatCount == 2) {
                // 시스템이 롱클릭으로 판단한 순간 즉시 B Activity 실행
                openRecorderFromPuiCapture();
                performCameraKeyHaptic();
                mFunctionKeyDown = false;
                return -1; // 이벤트 소비 (더 이상 전파되지 않음)
            }
        } 
        // 2. 카메라 버튼에서 손을 뗐을 때 (ACTION_UP)
        else if (action == KeyEvent.ACTION_UP) {
            // 롱클릭 플래그가 없고, 취소되지 않은 정상적인 숏클릭인 경우
            if (!isLongPress && mFunctionKeyDown && (event.getFlags() & KeyEvent.FLAG_CANCELED) == 0) {

              openCameraFromPuiCapture();
              performCameraKeyHaptic();
              return -1;
...
// QTI_END: 2021-04-28: Display: HDMI/DP pluggin notification changes
    ...
    private boolean shouldDispatchInputWhenNonInteractive(int displayId, int keyCode) {
        ...
        if ((keyCode == KeyEvent.KEYCODE_CAMERA) || (keyCode == KeyEvent.KEYCODE_FUNCTION))
        {
            return false;
        }
